...
const UnlockReportCTA: React.FC<UnlockReportCTAProps> = ({ onUnlock, isLoading }) => {
  return (
    <div className="bg-gradient-to-tr from-cosmic-gold/10 to-transparent p-6 rounded-2xl border-2 border-dashed border-cosmic-gold/50 text-center my-8 animate-fade-in no-print">
      ...
      <h3 className="text-2xl font-bold font-display text-starlight">Unlock Your Full Soul Map</h3>
      <p className="text-lunar-grey mt-2 mb-6 max-w-md mx-auto">You are viewing a summarized report. Unlock the complete, in-depth analysis for a spiritual operating manual to your destiny.</p>
      <button
        onClick={onUnlock}
        disabled={isLoading}
